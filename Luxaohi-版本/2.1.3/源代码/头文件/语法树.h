...
class 数组声明节点 : public 语法树节点 {
public:
    std::string 名称;
    size_t 大小 = 0;
    std::vector<std::string> 初始值;
    void 执行(解释器& 解释器) override;
};

class 数组元素赋值节点 : public 语法树节点 {
public:
    std::string 名称;
    std::string 索引表达式;
    std::string 值表达式;
...
class 当循环节点 : public 语法树节点 {
public:
    std::string 条件;
    std::vector<std::shared_ptr<语法树节点>> 循环体;
    void 执行(解释器& 解释器) override;
};

class 返回语句节点 : public 语法树节点 {
public:
    std::string 值;
    void 执行(解释器& 解释器) override;
};