...
std::string 解释器::求值(const std::string& 表达式) {
    ...
    // 检查是否包含运算符
    if (表达式.find_first_of("+-*/()") != std::string::npos) {
        // 尝试作为数学表达式求值
        try {
            // 检查是否只包含可解析为表达式的字符
            bool 可解析为表达式 = true;
            for (char c : 表达式) {
                if (!isdigit(c) && !::是字母字符(c) &&
                    c != '+' && c != '-' && c != '*' && c != '/' &&
                    c != '(' && c != ')' && c != ' ' && c != '=' && c != '!' && c != '>' && c != '<') {
                    可解析为表达式 = false;
                    break;
...
bool 解释器::求值条件(const std::string& 条件) {
    ...
    if (等于位置 != std::string::npos) {
        ...
        int 左边 = std::stoi(左字符串);
        int 右边 = std::stoi(右字符串);
        return 左边 >= 右边;
    }
...
std::string 解释器::求值表达式(const std::string& 表达式) {
    ...
    if (位置 != 清理表达式.size()) {
        throw std::runtime_error("表达式解析不完整");
    }
    return 结果;
}
...
std::string 解释器::解析加法(const std::string& 表达式, size_t& 位置) {
    ...
    while (位置 < 表达式.size()) {
        ...
        if (操作符 == '+' || 操作符 == '-') {
            ...
            if (!是整数(左边) || !是整数(右边)) {
                throw std::runtime_error("数学运算需要数字参数");
            }
            int 左值 = std::stoi(左边);
            int 右值 = std::stoi(右边);
...
std::string 解释器::解析乘法(const std::string& 表达式, size_t& 位置) {
    ...
    while (位置 < 表达式.size()) {
        ...
        if (操作符 == '*') {
            ...
            int 左值 = std::stoi(左边);
            int 右值 = std::stoi(右边);
            if (右值 == 0) throw std::runtime_error("不能除以零");
            左边 = std::to_string(左值 / 右值);
        }
        else break;
    }
...
std::string 解释器::解析基本项(const std::string& 表达式, size_t& 位置) {
    ...
    if (isdigit(表达式[位置])) {
        std::string 数字;
        while (位置 < 表达式.size() && isdigit(表达式[位置])) {
            数字 += 表达式[位置++];
        }
        return 数字;
    }
    ...
    if (::是字母字符(表达式[位置])) {
        ...
        // 变量未定义，抛出异常
        throw std::runtime_error("未定义变量: " + 变量);
    }
    ...
    // 遇到无法解析的字符，抛出异常
    throw std::runtime_error("无法解析的字符: " + std::string(1, 表达式[位置]));
}